...
class BaiduAI {
  ...
  initMicroPhone() {
    ...
    this._audioPromise.then(stream => {
      ...
      vm.emit(('showAlert', {message: '没有麦克风', type: 'warn'}));
    });
  ...
  getInfo() {
    return {
      ...
      blocks: [
        {
          ...
          text: 'Image API ID:[ID] Secret:[SECRET]',
          arguments: {
            ID: {
        ...
        '---',
        {
          opcode: 'ttspersonality',
          blockType: BlockType.COMMAND,
          text: 'tts Personality [PERSON]',
          arguments: {
            PERSON: {
              type: ArgumentType.STRING,
              defaultValue: '1',
        ...
        '---',
        {
          opcode: 'ocrRecognize',
          blockType: BlockType.COMMAND,
          text: 'OCR Type[TYPE] List[OUTPUT]',
          arguments: {
            TYPE: {
              type: ArgumentType.STRING,
              menu: 'ocrmenu',
      ...
      menus: {
        typemenu: ['object', 'ingredient', 'plant','dish','animal','redwine','car'],
        ttsperson: [
          {text: '度小宇', value: '1'},
          {text: '度小美', value: '0'},
          {text: '度逍遥', value: '3'},
        ...
        ocrmenu: ['common', 'carplate', 'handwriting']
      },
      translation_map: {
        'zh-cn': {
          ...
          ttspersonality: "tts人物 [PERSON]",
          text2speech: "tts文字转语音 [TEXT]",
          typemenu: {
              object: '主体',
              ingredient: "蔬菜",
              plant: "盆栽",
          ...
          ocrRecognize: "文字识别 类型[TYPE] 数组[OUTPUT]",
  ...
  imageAPI (args){
    return new Promise((resolve, reject) => {
      fetch(`https://openapi.baidu.com/oauth/2.0/token?grant_type=client_credentials&client_id=${args.ID}&client_secret=${args.SECRET}`, {
        method: "GET"
      }).then(res => res.json()).then(ret => {
        tokens.image = ret.access_token;
        resolve(tokens.image);
      }).catch(err => {
        vm.emit(('showAlert', {message: '更新token失败', type: 'warn'}));
      });
    })
  }
  ...
  ttspersonality (args, util){
    this.ttsperson = args.PERSON
  }

  text2speech (args, util){
    let txt = encodeURIComponent(args.TEXT);
    let url = `http://tsn.baidu.com/text2audio?tex=${txt}&tok=${tokens.voice}&cuid=kittenblock&ctp=1&lan=zh&per=${this.ttsperson || 0}`;
  ...
  recognizeResult (args, util){
    if (this._recognizeResult && this._recognizeResult.wineNameCn){
      ...
      return this._recognizeResult[0].name || this._recognizeResult[0].keyword;
    }
  }
  ...
  ocrRecognize (args, util){
    const regType = args.TYPE;
    const list = util.ioQuery('fs', 'getBlockListPointer', [util.blockid, 'OUTPUT']);
    if (!list){
      return Promise.reject('输出需要数组类型');
    }

    const img = util.ioQuery('fs', 'stageCanvasData', []).replace('data:image/png;base64,', '');
    const url = `${ocrMap[regType]}?access_token=${tokens.image}`;
    return new Promise((resolve, reject) => {
      fetch(url, {
        method: 'POST',
        headers: {
        ...
        if (ret.words_result && ret.words_result.length > 0){
          const words = [];
          for (let r of ret.words_result){
            words.push(r.words);
          }
          list.value = words;
          resolve(words)
        } else if (regType === "carplate" && ret.words_result && ret.words_result.number){
          list.value = [ret.words_result.number];
          resolve(ret.words_result.number)
        } else {
          resolve('')
        }
      }).catch(err => {
