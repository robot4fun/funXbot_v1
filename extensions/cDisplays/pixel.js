...
var ColorString = require('./color-string.js'); // used for color parsing
    ...
    FIRMATA: {
        initialize: {
            value: function(opts) {
                ...
                var data_pin = opts.pin || PIN_DEFAULT;
                //console.log("strip pin is", data_pin); //for debug
                //if (opts.pin) console.log("OR this one?", opts.pin); //for debug
                ...
                var port = firmata.transport; // 不需要, 可以呼叫firmata.sysexCommand()
                //console.log("ws2812 port is..", port); //for debug
                ...
                //console.log("PIXEL_CONFIG -> firmata..", data); //for debug
                firmata.sysexCommand(data);
        ...
        show: {
            value: function() {
                ...
                //data.push(START_SYSEX);
                data.push(PIXEL_COMMAND);
                data.push(PIXEL_SHOW);
                //data.push(END_SYSEX);

                // now just write that to the port and it should show the frame.
                //console.log("send PIXEL_SHOW to port..", data); //for debug
                //console.log("PIXEL_SHOW -> firmata", data); //for debug
                strip.firmata.sysexCommand(data);
        ...
        strip_color: {
            value: function(color) {
                ...
                //console.log("Send PIXEL_SET_STRIP command to firmata..", data); //for debug
                strip.firmata.sysexCommand(data);
                //strip.port.write(new Buffer(data));
            },
        },
        _shift: {
            value: function(amt, direction, wrap) {
                ...
                //data.push(START_SYSEX);
                data.push(PIXEL_COMMAND);
                data.push(PIXEL_SHIFT);
                data.push((amt | direction | wrap_val) & FIRMATA_7BIT_MASK);
                //data.push(END_SYSEX);

                //console.log("Send PIXEL_SHIFT command to firmata..", data); //for debug
                strip.firmata.sysexCommand(data);
                //strip.port.write(new Buffer(data));
            },
        },
...
function Strip(opts) {
    ...
    // pin: data pin for the pixel strip // DEPRECATED will be phased out.
    ...
    FIRMATA: {
        ...
        pixel_color: {
            value: function(color) {
                ...
                //console.log("PIXEL_SET_PIXEL no.", pixel.address); //for debug
                //console.log("PIXEL_SET_PIXEL -> firmata", data); //for debug
                pixel.firmata.sysexCommand(data);
