#include "funXbot_MX1508.h"

funXbotMX1508::funXbotMX1508( uint8_t pinIN1, uint8_t pinIN2) {
...
funXbotMX1508::funXbotMX1508( uint8_t pinIN1, uint8_t pinIN2, DecayMode decayMode, NumOfPwmPins numPins) {
  _pinIN1 = pinIN1; // always a PWM pin
  _pinIN2 = pinIN2; // can be a non-Pwm pin.
...
int16_t funXbotMX1508::getPWM() {
  return _pwmVal;
}

void funXbotMX1508::stopMotor() {
  digitalWrite(_pinIN1, LOW);
  digitalWrite(_pinIN2, LOW);
}

void funXbotMX1508::setResolution(uint16_t pwmResolution) {
  _pwmResolution = pwmResolution;
  if(_useAnalogWrite16) ICR1 = pwmResolution;
}

void funXbotMX1508::setPWM16(uint8_t prescaler, uint16_t resolution){
...
void funXbotMX1508::analogWrite16(uint8_t pin, uint16_t val)
{
...
void funXbotMX1508::motorGo(int16_t pwmSpeed) {
  _pwmVal = pwmSpeed;
  //pwmSpeed = constrain(pwmSpeed, (-1*_pwmResolution), _pwmResolution);
  ...
  if (this->_whichMode == FAST_DECAY) {
   ...
   // if decay mode is set as slow decay mode
    ...
    if (pwmSpeed >= 0) {                        // forward slow decay
      pwmSpeed = constrain(pwmSpeed, 0, _pwmResolution);
      pwmSpeed = map(pwmSpeed, 0, _pwmResolution, _pwmResolution, 0);
