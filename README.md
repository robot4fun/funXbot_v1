...
## `funXtcp` WiFi模組
  ...
  * 筆電WiFi連接 ESP12S模組 ( 預設SSID: `ESP_00xx` ) 後
    * 以瀏覽器連192.168.4.1 
    * 進入網頁後選 `Upgrade Firmware` 頁面 
    * 點選 `選擇檔案` 選取 [user2.bin](wifi/user2.bin)之後點選 `Update the firmware`
    * reload網頁後將 `serial LED` 改為 `GPIO2` 
    * 視情況更改 `Reset` 腳位，應該是 `GPIO13` 
    * 更新韌體回 [user1.bin](wifi/user1.bin) 
    * 點選 `Home` 頁面後更改 `Hostname` 為 `funXbot`
    * 點選 `WiFi Soft-AP` 頁面後更改 `SSID` 為 `funXtcp00xx` (以MAC當序號)
    * 更改 `Password` 為 `12345678`
...
## `其他設備`
* RJ11 `6P4C` 水晶頭會連接母座的 `2-5腳位`，RJ11 `6P6C` 水晶頭會連接母座的 `1-6腳位` ![](doc/hardware/A6A7BT1.jpg)
* 除電源和接地外，請按以下說明接線:
...
* `8路循線感應器`: 由 `QTR-8A 循線模組` 和 `CD74HC4067 類比IO擴展模組` 組成 ![](doc/hardware/ls8.png) 將兩者的對應腳位連接後，使用 `6P6C` 水晶頭將 CD74HC4067 類比IO擴展模組的 `s0` 接 `腳位1`， `s1` 接 `腳位2`， `s2` 接 `腳位3`， `SIG` 需額外的 `6P4C` 水晶頭，接 `腳位3` 。
...
* `顯示設備`:
  ...
  * 燈條或燈環訊號接 `腳位3`
  * 將兩個 MAX7219 LED 顯示板拼接後，使用 `6P6C` 水晶頭， `clk` 接 `腳位1` ， `cs` 接 `腳位2` ， `din` 接 `腳位3`

* `馬達`:
  * 伺服馬達接 `腳位3`
  * 360連續旋轉伺服馬達接 `腳位2`  
  * 直流馬達需接入 `MX1508 直流馬達驅動板` ![](doc/hardware/mx1508.jpeg) 1張可驅動2台馬達，因為便宜，每張驅動1台馬達就好。將其黏接在TT馬達上，依圖說接線， `IN1` 接 `腳位2` ， `IN2` 接 `腳位3` 
  * 如果要同時驅動2台直流馬達，須將MX1508驅動板的 `MOTOR` 接 USB `MicroB` 或 `TypeC` 母座，馬達接USB公頭，較為方便![](doc/hardware/2motors.jpg)
...
4. funXblock程式積木所指x軸為朝接口5-8方向; y軸朝向USB及電源接口方向![xyz.jpg](doc/hardware/xyz.jpg)

5. 安插 funXtcp WiFi 模組時, 白色標記需和主控器無線通訊接口白色標記對齊![wifi.jpg](doc/hardware/wifi.jpg)
...
9. `16路循線感應器` `控制接口` 需選 `3` ![](doc/software/ls16.png)
